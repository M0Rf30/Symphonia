...
pub(crate) fn read_header(mss: &mut MediaSourceStream<'_>) -> Result<(StreamInfo, Sv8State)> {
    ...
    loop {
        ...
        if &key == b"AP" || &key == b"SE" {
            ...
            return Ok((info, Sv8State { data_start: block_start }));
        }
        ...
        match &key {
            ...
            // "SO"/"ST" (seek table location/table): parsed only enough to skip past; this
            // crate falls back to linear block scanning for seeking (see `seek` below) rather
            // than the seek-table fast path, a documented simplification (see crate docs).
            _ => {}
        }
    }
}
...
impl Sv8State {
    ...
    pub fn seek(
        ...
        mss.seek(std::io::SeekFrom::Start(self.data_start)).map_err(symphonia_core::errors::Error::IoError)?;
