...
use super::data::{PsConfig, PsData, PsIndexState};
use super::decorr::PsDecorr;
use super::hybrid::{synthesize, HybridConfig, PsHybrid};
use super::stereo::PsStereo;
use super::super::qmf::Complex;
use super::super::error::SbrResult as Result;
...
mod tests {
    use super::*;
    use super::super::hybrid::{LOOKAHEAD, NUM_QMF_SLOTS};
    use super::super::super::bits::BitWriter;
    ...
    fn payload(iid_idx: i32) -> Vec<u8> {
        ...
        let (len, code) = super::super::huffman::HUFF_IID_DF[(iid_idx + 14) as usize];
        w.write_u32(code, u32::from(len));
        let (l0, c0) = super::super::huffman::HUFF_IID_DF[14];
        for _ in 1..10 {
            w.write_u32(c0, u32::from(l0));
        }
        w.write_bit(false); // icc_dt = freq
        let (li, ci) = super::super::huffman::HUFF_ICC_DF[7];
        for _ in 0..10 {
    ...
    /// Mutation-fuzz test: flips/replaces bytes throughout a real
    /// header'd `ps_data()` payload (built the same way as
    /// [`payload`]) and feeds it to [`PsDecoder::process`] alongside
    /// truncated / random byte buffers of similar length, asserting
    /// the decoder never panics regardless of how malformed the
    /// bitstream is. `SbrDecoder`/`PsDecoder` are reached from
    /// untrusted network packets (see `symphonia-codec-aac/tests/
    /// tests.rs`'s equivalent multichannel/PCE fuzz tests), so a
    /// malformed PS payload must surface as `Err`, never a panic.
    #[test]
    fn fuzz_ps_data_mutation_never_panics() {
        use std::panic::{self, AssertUnwindSafe};

        let fixture = payload(7);
        let x = x_input_ones();
        ...
        for trial in 0..4000u32 {
            ...
            let mutations = 1 + (next() % 5) as usize;
            ...
            if trial % 6 == 0 && !mutated.is_empty() {
                let len = 1 + (next() as usize) % mutated.len();
                mutated.truncate(len);
            }
            if trial % 9 == 0 {
                let len = 1 + (next() as usize) % (fixture.len().max(1) * 2);
                mutated = (0..len).map(|_| next() as u8).collect();
            }

            let result = panic::catch_unwind(AssertUnwindSafe(|| {
                let mut dec = PsDecoder::new();
                // Prime with a valid header first (mirrors real streams: PS is
                // inactive/mono until the first decodable header), then hand
                // the mutated bytes to every subsequent frame.
                let _ = dec.process(Some(&fixture), &x, 32);
                for _ in 0..3 {
                    let _ = dec.process(Some(&mutated), &x, 32);
                }
            }));
            assert!(result.is_ok(), "PsDecoder panicked on mutated ps_data (trial {trial})");
        }
    }
}
