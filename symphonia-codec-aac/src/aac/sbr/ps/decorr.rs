...
use super::map::parameter_map;
use super::super::qmf::Complex;
use super::super::error::{SbrError as Error, SbrResult as Result};
...
pub struct PsDecorr {
    ...
    /// Per-slot transient-detector scratch (§8.6.4.5.3 `p`/`gRatio`),
    /// reused across [`Self::process`] calls instead of allocating
    /// fresh `Vec`s every slot — the hot path for every PS-active
    /// frame (32 slots/frame).
    scratch_p: Vec<f64>,
    scratch_g_ratio: Vec<f64>,
}
...
impl PsDecorr {
    ...
    pub fn new(config: HybridConfig) -> Self {
        ...
        PsDecorr {
            ...
            scratch_p: vec![0.0; c.nr_par_bands],
            scratch_g_ratio: vec![1.0; c.nr_par_bands],
        }
    }
    ...
    pub fn process(&mut self, s: &[Vec<Complex>]) -> Result<Vec<Vec<Complex>>> {
        ...
        for (n, row) in s.iter().enumerate() {
            // §8.6.4.5.3 transient detection at this slot. `scratch_p` /
            // `scratch_g_ratio` are reused across calls (sized once, in
            // `new()`) instead of allocating a fresh `Vec` per slot.
            self.scratch_p.fill(0.0);
            for (k, v) in row.iter().enumerate() {
                self.scratch_p[usize::from(b_k[k])] += v.norm_sqr();
            }
            self.scratch_g_ratio.fill(1.0);
            for i in 0..c.nr_par_bands {
                let p_i = self.scratch_p[i];
                let peak = if PEAK_DECAY * self.peak_decay_nrg[i] < p_i {
                    p_i
                } else {
                    PEAK_DECAY * self.peak_decay_nrg[i]
                };
                self.peak_decay_nrg[i] = peak;
                self.smooth_nrg[i] += A_SMOOTH * (p_i - self.smooth_nrg[i]);
                self.smooth_peak_diff[i] += A_SMOOTH * (peak - p_i - self.smooth_peak_diff[i]);
                if GAMMA * self.smooth_peak_diff[i] > self.smooth_nrg[i] {
                    self.scratch_g_ratio[i] = self.smooth_nrg[i] / (GAMMA * self.smooth_peak_diff[i]);
                }
            }
            ...
            for k in 0..c.nr_allpass_bands {
                ...
                out[n][k] = u * self.scratch_g_ratio[usize::from(b_k[k])];
            }
            ...
            for k in c.nr_allpass_bands..c.nr_bands {
                ...
                out[n][k] = v * self.scratch_g_ratio[usize::from(b_k[k])];
            }
        }
        Ok(out)
    }
...
mod tests {
    use super::*;
    use super::super::hybrid::HybridConfig;
