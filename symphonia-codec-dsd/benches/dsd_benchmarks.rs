    ...
    for &size in packet_sizes.iter() {
        ...
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                let mut decoder = symphonia_codec_dsd::DsdDecoder::try_new(&params, &DecoderOptions::default())
                    .unwrap();

                let data = vec![0x55u8; size];
                let packet = Packet::new_from_boxed_slice(0, 0, size as u64, data.into_boxed_slice());

                b.iter(|| {
                    decoder.reset();
                    black_box(decoder.decode(&packet).unwrap())
                });
            },
        );
    }
    ...
    for (name, dsd_rate, pcm_rate) in configs.iter() {
        ...
        group.bench_with_input(
            ...
            |b, &(dsd_rate, pcm_rate, packet_size)| {
                ...
                let packet = Packet::new_from_boxed_slice(0, 0, packet_size as u64, data.into_boxed_slice());
