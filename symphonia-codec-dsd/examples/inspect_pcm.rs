...
use symphonia_core::codecs::{DecoderOptions, CODEC_TYPE_NULL, Decoder};
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: {} <dsd_file>", args[0]);
        std::process::exit(1);
    }
    ...
    let input_rate = codec_params.sample_rate.expect("No sample rate");
    let channels = codec_params.channels.expect("No channels");
    ...
    // Create DSD decoder directly
    let mut decoder = symphonia_codec_dsd::DsdDecoder::try_new(&params_with_pcm, &DecoderOptions::default())
