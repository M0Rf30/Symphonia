...
pub fn get_word1(ws: &mut WordState, bits: &mut Bits<'_>, chan: usize, flags: i16) -> i32 {
    ...
    if ones_count == 0 {
        ...
        if ones_count == 1 {
            ...
            let tmp2 = bitset(k1);
            let raw_av: u32 = if ones_count == 24 {
                bits.getbits(24) & 0xffffff
            } else {
                let k2 = {
                ...
                (raw & bitmask(k2)).wrapping_add(bitset(k2).wrapping_mul(ones_count - 2))
            };
...
pub fn get_word3(ws: &mut WordState, bits: &mut Bits<'_>, chan: usize, wphdr_bits: i16) -> i32 {
    ...
    let delta_dbits =
        if (cbits + 1) % 3 == 0 { (cbits + 1) / 3 } else { -(cbits - cbits / 3) };
