...
#[test]
fn track_duration_reflects_sum_of_all_blocks() {
    // Three 50-frame stereo blocks; the RIFF/WAVE wrapper has no field for the
    // aggregate sample count, so the reader must derive it by walking every
    // block header.
    let b = wvpk_v3_block(2, 2, 50, &[0u8; 4]);
    let data = build_riff(2, 44100, 2, &[], &[b.clone(), b.clone(), b.clone()]);
    let reader = open(data);

    let track = &reader.tracks()[0];
    assert_eq!(track.num_frames, Some(150), "num_frames must sum every block's total_samples");
    assert_eq!(track.duration, Some(Duration::new(150)));
    assert_eq!(reader.media_info().duration, Some(Duration::new(150)));
}

#[test]
fn seek_lands_on_block_containing_timestamp() {
    use symphonia_core::formats::{SeekMode, SeekTo};

    let b1 = wvpk_v3_block(2, 2, 50, &[0u8; 4]);
    let b2 = wvpk_v3_block(2, 2, 50, &[0u8; 4]);
    let b3 = wvpk_v3_block(2, 2, 50, &[0u8; 4]);
    let data = build_riff(2, 44100, 2, &[], &[b1, b2, b3]);
    let mut reader = open(data);

    let seeked = reader
        .seek(SeekMode::Accurate, SeekTo::Timestamp { ts: Timestamp::new(75), track_id: 0 })
        .expect("seek should succeed");
    assert_eq!(seeked.actual_ts.get(), 50, "frame 75 lives in the second 50-frame block");

    let pkt = reader.next_packet().unwrap().expect("packet after seek");
    assert_eq!(pkt.pts.get(), 50);
    assert_eq!(pkt.dur.get(), 50);
}
...
fn seek_past_end_of_stream_errors() {
    ...
    let b = wvpk_v3_block(2, 2, 50, &[0u8; 4]);
    let data = build_riff(2, 44100, 2, &[], &[b]);
    let mut reader = open(data);

    let result = reader.seek(SeekMode::Accurate, SeekTo::Timestamp { ts: Timestamp::new(1000), track_id: 0 });
    assert!(result.is_err(), "seeking beyond num_frames must fail");
}
