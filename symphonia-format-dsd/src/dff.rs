...
/// Validate channel IDs according to DSDIFF specification
fn validate_channel_ids(channels: &[[u8; 4]], count: u16) -> Result<()> {
    // Valid channel IDs from DSDIFF 1.5 spec
    const VALID_CHANNEL_IDS: &[&[u8; 4]] = &[
        b"SLFT", // Stereo Left
        b"SRGT", // Stereo Right
        b"MLFT", // Multi-channel Left Front
        b"MRGT", // Multi-channel Right Front
        b"C   ", // Center
        b"LFE ", // Low Frequency Effects
        b"LS  ", // Left Surround
        b"RS  ", // Right Surround
        b"SL  ", // Surround Left
        b"SR  ", // Surround Right
    ...
    for ch_id in channels {
        if !VALID_CHANNEL_IDS.contains(&ch_id) {
            warn!(
                "DFF: Unknown channel ID: {}",
                String::from_utf8_lossy(ch_id)
            );
        }
    }

    // Validate common channel orderings
    match count {
        2 => {
            // Stereo: SLFT, SRGT
        ...
        6 => {
            // 5.1: MLFT, MRGT, C, LFE, LS, RS (most common ordering)
...
impl FormatReader for DffReader {
    fn try_new(mut source: MediaSourceStream, _options: &FormatOptions) -> Result<Self> {
        ...
        // Create INFO parser for metadata
        let mut info_parser = DffInfoParser::new();

        // Find DSD audio data chunk and parse INFO chunks
        ...
        // Build metadata log from INFO parser
        let mut metadata_log = MetadataLog::default();
        let info_metadata = info_parser.into_metadata().metadata();
        if !info_metadata.tags().is_empty() || !info_metadata.visuals().is_empty() {
            metadata_log.push(info_metadata);
        }
