...
/// Read ID3v2 metadata from DSF file
fn read_dsf_metadata(reader: &mut MediaSourceStream, metadata_pointer: u64, metadata_log: &mut MetadataLog) {
    ...
    let metadata_result = symphonia_metadata::id3v2::read_id3v2(reader, &mut metadata_builder);

    // Restore position to data section
...
impl DsfFormatChunk {
    ...
    fn validate(&self) -> Result<()> {
        ...
        // Validate block size to prevent division by zero
        if self.block_size_per_channel == 0 {
            return decode_error("dsf: invalid block size (zero)");
        }

        // Validate total block size doesn't overflow
...
impl FormatReader for DsfReader {
    ...
    fn seek(&mut self, _mode: SeekMode, to: SeekTo) -> Result<SeekedTo> {
        ...
        debug_assert!(
            required_block < self.total_blocks,
            "seek target exceeds total_blocks"
        );
